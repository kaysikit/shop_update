...
class ProductFeatures(models.Model):
    ...
    postfix_for_value = models.CharField(
        ...
        help_text=f'Например для характеристики "Часы работы" к ' 
                  f'значению можно добавить постфикс "часов", и как результат - значение "10 чаоов"'
    )
    ...
    filter_measure = models.CharField(
        max_length=50,
        verbose_name='Единицы измерения для фильтра',
        help_text='Единица измерения для конкретного фильтра. Например "Частота процессора (Ghz). '
                  'Единицы измерения будет информация в скобках"'
    )
...
class ProductFeautureValidators(models.Model):
    ...
    def __str__(self):
        ...
        return f'Валидатор категории "{self.category.name} | ' \
               ...
               f'Значения - "{self.feauture_value}"'
